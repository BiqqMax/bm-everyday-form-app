...
function toPublicFormView(form: PublicFormRow, ownerDisplayName: string | null, fields: PublicFormField[]): PublicFormView {
  return {
    id: form.id,
    title: form.title,
        ...
        <div className="w-full overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-[0_20px_60px_rgba(15,23,42,0.08)]">
          <div className="border-b border-slate-100 bg-gradient-to-r from-slate-900 to-slate-700 px-6 py-8 text-white sm:px-10">
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2">
              {displayName ? <span className="rounded-full border border-white/20 bg-white/10 px-3 py-1 text-xs font-medium text-slate-100">Shared by {displayName}</span> : null}
              {form?.description ? <p className="max-w-2xl text-sm leading-6 text-slate-200 sm:text-base">{form.description}</p> : null}
            </div>
          </div>
